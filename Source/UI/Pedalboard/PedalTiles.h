...
public:
        ...
        : PedalTileComponent (apvtsIn, "amp", "Amp", "ampOn")
    {
        ...
        voiceCombo = makeTileCombo (*this, apvtsIn, "ampVoice", "", { "Vintage 5E3", "Boutique", "Vox Top Boost", "Deluxe 63", "JTM45", "Mark I" });
...
protected:
    void resizedBody (juce::Rectangle<int> body) override
    {
        ...
        if (voiceIndex == 1 || voiceIndex == 3 || voiceIndex == 4 || voiceIndex == 5)
...
private:
    ...
    {
        const auto nowVoiceIndex = juce::jlimit (0, 5, (int) apvts.getRawParameterValue ("ampVoice")->load());
        if (! force && nowVoiceIndex == voiceIndex)
            return;
        voiceIndex = nowVoiceIndex;
        const auto showTone = voiceIndex == 0;
        const auto showBass = voiceIndex == 1 || voiceIndex == 2 || voiceIndex == 3 || voiceIndex == 4 || voiceIndex == 5;
        const auto showMid = voiceIndex == 1 || voiceIndex == 3 || voiceIndex == 4 || voiceIndex == 5;
        const auto showTreble = voiceIndex == 1 || voiceIndex == 2 || voiceIndex == 3 || voiceIndex == 4 || voiceIndex == 5;
