...
{
public:
    TS9Node (juce::AudioProcessorValueTreeState& state, ProcessingQualityState& quality)
    ...
    void prepare (const juce::dsp::ProcessSpec& spec) override
    {
        ...
        prepareCrossfade (spec, pBool ("preFxSectionOn") && pBool ("ts9On"));
    }
    void reset() override { for (auto& t : ts9s) t.reset(); }
    void oversamplingModeChanged() override { for (auto& t : ts9s) t.reset(); }
    ...
    {
        return ts9s[(size_t) oversamplingMode()].getLatencySamples();
    }
    ...
    bool updateAndProcess (juce::AudioBuffer<float>& buffer, bool inTargetOrder) override
    {
        ...
        auto& selected = ts9s[(size_t) oversamplingMode()];
